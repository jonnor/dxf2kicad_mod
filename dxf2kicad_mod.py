...
def cmds_from_entities(entities):
    ...
    return functools.reduce(lambda x, y: x+y,
                            (list(fp_polys(layer, entities))
                             ...
                             _layer_entities(entities)))


# Resolve INSERT entities, which refers to reusable BLOCKs
def resolve_blocks(dxf, entities):
    ret = []
    for e in entities:
        if e.dxftype in ['INSERT']:
            b = dxf.blocks[e.name]
            r = resolve_blocks(dxf, b)
            for e in r:
                ret.append(e)
        else:
            ret.append(e)
...
def main():
    if len(sys.argv) < 2:
        ...
        entities = resolve_blocks(dxf, dxf.entities)

        print(str(kf.Module('autogenerated',
                            children=cmds_from_entities(entities))))
